...
test('Renders <FormResult /> when currentJobId is populated', () => {
  render(<FormResult />, {initialState: stateWithSuccessfulResult})

  const formResultComponent = screen.getByTestId('formResult')
  
  expect(formResultComponent).toBeInTheDocument()
})

test('Displays Loading correctly when processing jobs', () => {
  render(<FormResult />, {initialState: stateWithProcessingJob})

  const findingSubscriberMessage = screen.getByText('Finding Subscriber...')
  const loadingMessage = screen.getByTestId('spinner-message').textContent
  const loader = screen.getByText('Main Frame Loading...')

  expect(findingSubscriberMessage).toBeInTheDocument()
  expect(loadingMessage).toEqual('Hold tight! We are finding balwill@bu.edu for you')
  expect(loader).toBeInTheDocument()
})

test('Displays failed job with details', () => {
  render(<FormResult />, {initialState: stateWithFailedJob})

  const failMessage = screen.getByText('Hmmm...Looks like this job failed. The following was the reason provided:')
  const failDetails = screen.getByText('The job failed', {exact: false})

  expect(failMessage).toBeInTheDocument()
  expect(failDetails).toBeInTheDocument()
})

test('Renders <SubscriberInfoResults /> and <DataExtensionTables /> when job is completed successfully', () => {
  render(<FormResult />, {initialState: stateWithSuccessfulResult})
  
  const subscriberInfoResultsComponent = screen.getByTestId('subscriberInfoResults')
  const dataExtensionTablesComponent = screen.getByTestId('data-extension-results-section')

  expect(subscriberInfoResultsComponent).toBeInTheDocument()
  expect(dataExtensionTablesComponent).toBeInTheDocument()
})